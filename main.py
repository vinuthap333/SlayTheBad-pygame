...
def show_go_screen():
    ...
    draw_text(screen , "SLAY THE BAD",64,WIDTH/2, HEIGHT/4)
    draw_text(screen , "Arrow keys to move, Space to fire", 22 ,WIDTH/2, HEIGHT/2)
    draw_text(screen , "Press any key to start, Go get them!", 18 ,WIDTH/2, HEIGHT *3/4)
    pygame.display.flip()
    waiting = True
    while waiting:
        clock.tick(FPS)
...
class Mob(pygame.sprite.Sprite):
    def __init__(self):
        pygame.sprite.Sprite.__init__(self)
        self.image_orig = random.choice(mob_list)
...
background = pygame.image.load(path.join(img_dir,'background_r.png')).convert()
...
mob_list = []
mob_imgs = ['mob1.png','mob2.png','mob3.png']
for img in mob_imgs:
    mob_list.append(pygame.image.load(path.join(img_dir,img)).convert())
...
shield_sound = pygame.mixer.Sound(path.join(snd_dir,'Powerup5.wav'))
power_sound = pygame.mixer.Sound(path.join(snd_dir,'Powerup24.wav'))
...
while running:
    ...
    #if the player died and explosion has finished playing
